    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
        ...
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form action="#" method="POST" class="space-y-6" @submit="onSubmit">
                ...
                <div>
                    <div v-if="error.length > 0" class="text-sm/6 font-medium text-red-600 mb-3">
                        {{ error }}
                    </div>
                    <button
                        type="submit"
            ...
            <p class="mt-10 text-center text-sm/6 text-gray-400">
                Not a member?
                <router-link :to="{name: 'sign-up'}" :class="'font-semibold text-indigo-400 hover:text-indigo-300'">Sign
                    up
                </router-link>
            </p>
        </div>
